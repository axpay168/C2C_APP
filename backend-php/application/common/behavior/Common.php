...
class Common
{
    ...
    {
        // 後台入口（buSRxMqJOo.php）已綁定 admin 模塊，不要因 pathinfo 含 index 而切到 index 模塊
        $script = $_SERVER['SCRIPT_NAME'] ?? '';
        if (strpos($script, 'buSRxMqJOo') !== false) {
            return;
        }
    ...
    public function moduleInit(&$request)
    {
        ...
        if (Config::get('lang_switch_on')) {
            ...
            if (is_string($lang) && preg_match("/^([a-zA-Z\-_]{2,10})\$/i", $lang)) {
    ...
    public function addonBegin(&$request)
    {
        ...
        $lang = (is_string($lang) && preg_match("/^([a-zA-Z\-_]{2,10})\$/i", $lang)) ? $lang : 'zh-cn';
        Lang::load([
